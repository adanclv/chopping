...
export default function TablaProveedor({ proveedores }) {
    const navigate = useNavigate();

    const handleEdit = (id) => {
        navigate(`/admin/proveedores/edit/${id}`);
    };

    return (
        <div className="container-table-admin">
            <div className="tabla-6col header-tabla">
                <p>Id</p>
                <p>Nombre</p>
                <p>Email</p>
                <p>Telefono</p>
            ...
            {proveedores && proveedores.map(proveedor => (
                <div className="tabla-6col content-tabla" key={proveedor.NoProveedor}>
                    <p>{proveedor.NoProveedor}</p>
                    <p>{proveedor.NombProveedor}</p>
                    <p>{proveedor.MailProveedor}</p>
                    <p>{proveedor.TelProveedor}</p>
                    <p className={'estado ' + (proveedor.Estado === 'activo' ? 'activo' : 'inactivo')}></p>
                    <div className="btn-action">
                        <button onClick={() => handleEdit(proveedor.NoProveedor)}>
